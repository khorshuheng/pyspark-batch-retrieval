...
def test_join_without_max_age(
    ...
    expected_joined_schema = StructType(
        [
            StructField("customer_id", IntegerType()),
            StructField("event_timestamp", TimestampType()),
            StructField("transactions_event_timestamp", TimestampType()),
            StructField("daily_transactions", FloatType()),
        ]
    )
    expected_joined_data = [
        (1001, datetime(year=2020, month=8, day=31), None, None),
        (
            1001,
            datetime(year=2020, month=9, day=1),
            datetime(year=2020, month=9, day=1),
        ...
        (3001, datetime(year=2020, month=9, day=1), None, None),
    ]
...
def test_join_with_max_age(
    ...
    expected_joined_data = [
        ...
        (1001, datetime(year=2020, month=9, day=3), None, None),
        (
            2001,
            datetime(year=2020, month=9, day=2),
            datetime(year=2020, month=9, day=1),
...
def test_join_with_composite_entity(
    ...
    expected_joined_schema = StructType(
        [
            ...
            StructField("driver_id", IntegerType()),
            StructField("event_timestamp", TimestampType()),
            StructField("ratings_event_timestamp", TimestampType()),
            StructField("customer_rating", FloatType()),
            StructField("driver_rating", FloatType()),
        ]
    )
    expected_joined_data = [
        ...
        (1001, 8002, datetime(year=2020, month=9, day=3), None, None, None),
        (1001, 8003, datetime(year=2020, month=9, day=1), None, None, None),
        (
            2001,
            8001,
            datetime(year=2020, month=9, day=2),
...
def test_multiple_join(
    ...
    joined_df = query(query_conf, entity_df, tables)
    ...
    expected_joined_schema = StructType(
        [
            ...
            StructField("transactions_event_timestamp", TimestampType()),
            StructField("daily_transactions", FloatType()),
            StructField("bookings_event_timestamp", TimestampType()),
            StructField("completed_bookings", IntegerType()),
        ]
    )
...
def test_multiple_join_integration(spark):
    ...
    joined_df = batch_retrieval_dataframe(spark, batch_retrieval_conf)
